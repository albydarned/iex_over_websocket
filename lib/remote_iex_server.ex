...
defmodule Remote.IEx.Server do
  @moduledoc false
  ...
  def start_evaluator(opts) do
    evaluator =
      opts[:evaluator] ||
        :proc_lib.start(Remote.IEx.Evaluator, :init, [:ack, self(), Process.group_leader(), opts])
  ...
  defp iex_state(opts) do
    ...
    puts = Keyword.get(opts, :puts, &IO.puts/2)
    %Remote.IEx.State{prefix: prefix, gets: gets, puts: puts}
  end
